...
docs_to_table(const std::vector<yyjson_doc *> &docs,
  ...
  for (yyjson_doc *d : docs) envs.push_back(unpack(d));
  ...
  for (size_t c = 0; c < n_col; ++c) {
    if (drop.count(names[c])) continue;
