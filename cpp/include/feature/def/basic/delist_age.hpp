...
//   PIT: 不写"距事件天数" (未来信息). 下游用 is_finite 判 "已退市".

namespace feature::def {

inline void ts_delist_age(int a, const Axes &axes, const PitPool &pool,
...
inline constexpr FeatureSpec delist_age_spec{
    ...
    /*assumption=*/
    "[日历日]; 仅退市当日及之后写值 (退市当日=0), 否则 NaN; 下游 is_finite "
    "判\"已退市\""};

inline void ts_delist_age(int a, const Axes &axes, const PitPool &,
                          const StockMeta &meta, Tensor &T) {
  int n_d = axes.n_d();
  auto out = T.ts_row(delist_age_spec, a);
  const std::string &anchor = meta.delist_date[a];
  if (anchor.size() != 8) {
