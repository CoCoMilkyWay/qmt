...
//
// per-itf:
//   1) 算 cache_key (FNV: POOL_VERSION + itf.file_name + dayfile relpath/size/mtime
//      + axes _meta 文件内容). axes 由 trading_days/basic_info 推出.
//   2) 试 hit: mmap data/pool/<itf>.bin → 校验 magic/version/key → cache_layout(Map)
//      把 pool 字段 PoolArr 视图指向 mmap 区 (零反序列化 / 零 copy).
//   3) miss: itf.build(axes, files, pool) 端到端从 dayfile JSON 并行写入 pool;
