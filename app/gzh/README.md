...
│   ├── wxmd.hpp                            # 顶层门面: render_article_markdown(raw, 图片钩子) / render_article_html(raw)
│   │                                       #   标题/作者/公众号/时间/原文链接由 renderer 直接渲进正文, 不再另开一套结构
│   ├── sync.hpp                            # 主 flow: sync_account(root, account) — 先补齐再发现, 见「流程」一节
│   ├── fetch.hpp                           # fetch_raw(url, cookie) → 响应体; fetch_asset(url, seq) → AssetFile
...
│   ├── sync.cpp                            # 主 flow: download (抓一篇, 原子入库) / drain (排空队列) / discover (增量翻页)
│   │                                       #   / sync_account (先补齐再发现); 抓正文间隔写死在文件顶部
...
│                                           #   --html 只输出中间态 HTML | --uninstall 卸载
│                                           #   单篇输出打到标准输出, 要落盘用 shell 重定向
...
└── store/                                  # 本地缓存 (gitignore, --store 可换目录); 一个子目录一个公众号
    └── <__biz>/
...
**一条约束决定了这个形状**: 单篇文章页是公开 URL, 直连即可; 但**历史消息列表**
(`mp/profile_ext`) 必须带微信客户端侧的 `key` / `pass_ticket`, 只在微信内打开文章时才
下发, 按 `__biz` 绑定、分钟级过期 (按上游取 25 分钟), 程序无法自行续签 —— 所以「发现新
文章」做不到零交互, 而「补齐已发现的正文」完全不需要凭证。下面把两段、五级渲染流水线、
凭证捕获与本地缓存的原子性全部串成一份伪代码, 括号里标实际文件:

```
main(argv):
    if argv 给了 <链接> 或 -f <文件>:                            # 单篇模式, 不入缓存
        html = from_file ? read_file(path) : fetch_raw(url)       (fetch.cpp)
        print(html_only ? render_article_html(html)
                         : render_article_markdown(html, on_asset=空))   (wxmd.cpp)
        return
    run_sync(store_root, force_add)                                (cli/main.cpp)

run_sync(root, force_add):
    ...
    if not force_add and not confirm("加新号/刷新凭证?"):
        return
    fresh = capture_session(creds)                      # 段二: 抓包收新凭证, 见下
    creds.save()                                        # 覆盖同号旧凭证, 其余保留
...
drain(store):
    for entry in store.pending():         # 按发布时间升序, 逐篇来
        html = fetch_raw(entry.link)                       (fetch.cpp)
        status = validate_html(html)      # 五级流水线的第 2 级: 先判文章是否可用  (html.cpp)
        if status != Success:             # 已删除/违规/风控, 永远抓不回来
            store.commit(entry, status)   # 只留墓碑: 不建目录, 索引照样推进, 不然流水线卡死在这一篇
            continue
        assets = []
        markdown = render_article_markdown(html,           (wxmd.cpp 五级门面)
            on_asset = url -> { 抓这张图存进 assets; return "assets/NNN.ext" })   (assets.cpp)
        # render_article_markdown 内部: extract_cgi_script (2定位)
        #                             -> eval_cgi 得 cgiDataNew (3求值, 真 JS 引擎, JsDecode+\xNN 双层转义)
        #                             -> render_html 拼标题/元信息/原文链接/正文 (4渲染, 按 item_show_type 分支)
        #                             -> [on_asset 非空时] localize_images 改写 <img src> (可选插入点)
        #                             -> to_markdown (5转换, turndown 规则集移植)
        store.commit(entry, markdown, assets)

discover(store, account):
    since = store.watermark()             # 索引最后一行的发布时间, 空库为 0
...
AccountStore.commit(entry, markdown="", assets=[]):
    assert !store.has(entry.id)                        # 不重复入库
    assert entry.status 非空                             # 入库必带状态
    assert entry.datetime >= store.watermark()          # 必须从早到晚, 乱序当场炸
    if markdown 非空:
...
./run.py "<url>" > out.md                            # 单篇落盘 (shell 重定向)
...
`--html` 只对单篇有效, 同步模式下会喊一声然后忽略: 同步的产物就是缓存目录本身.
