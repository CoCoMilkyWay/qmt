...
| inter  | up_lim      | 时序 | itf:stk_limit                         | `stk_limit.up_limit` (前复权); ≥100000 为无涨停限制哨兵                                                                   | [元/股]; offset=-1 与 close_raw 对齐                                                                                                                                                  |
| inter  | dn_lim      | 时序 | itf:stk_limit                         | `stk_limit.down_limit` (前复权); ≤0.01 为无跌停限制哨兵                                                                   | [元/股]; offset=-1 与 close_raw 对齐                                                                                                                                                  |
...
Phase 2 时序  (per-A 并行; ts.cpp 通用 flow + feature.cpp 单点 feature 表)
  ...
  #     up_lim / dn_lim ← stk_limit[a, d-1] (保留哨兵 100000/0.01); susp ← suspend_d[a,d]   # offset = 0/-1
  #   ttm4         (财报 ytd 拼接; ts.hpp::ttm4_ytd_compute 模板, 自动降级):
