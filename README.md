...
| factor | pb_ttm1      | 截面 | pb_raw                                                                             | `pct_rank(z(winsor_mad(1 / pb_raw)))` + 截面均值填充                                                                                                                                                                                                   | —                                                                                                                                                                                                                                                                                                           |
...
| inter  | pb_raw       | 时序 | mcap_raw, itf:cn_stock_financial_balance_general_pit                               | `mcap_raw / balance.total_owner_equity` (取 latest report_date 的 latest visible 行)                                                                                                                                                                   | [ratio]; ttm1 (瞬时估值 / MRQ); 用含少数股东 `total_owner_equity` (≠ parent equity; 实测 BigQuant `cn_stock_valuation.pb` 用这个口径 — CATL 11% 误差 / 茅台 3-5% 误差 排查得). 同 visible_date 多 report_date 取 max(report_date); 分母 ≤ 0 → NaN                                                           |
...
Phase 3 截面  (per-D 并行; cs.cpp 通用 flow + feature.cpp 单点 feature 表)
  ...
  #   close (← close_raw, invert), mcap, fmcap, pe_ttm12, pb_ttm1, ps_ttm12, pcf_ttm12 (全 invert)
